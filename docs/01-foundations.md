...
This handbook focuses on **extractive video summarization**: selecting frames or temporally contiguous shots from an input video without training the selector against human importance scores or human summaries.
...
Abstractive video-to-text summaries, highlight detection, trailer generation, temporal action localization, and video question answering are adjacent tasks. They belong in a result table only when the prediction target and evaluation unit are made explicit.
...
| **Supervised comparator** | Human importance or summary labels train the selector; useful as context, but not part of an unsupervised leaderboard. |
...
| $\ell_j=|C_j|$ | Segment duration in original frames |
...
## 3. Strict four-paradigm taxonomy
...
These are canonical formulations. The [reconstruction chapter](04-reconstruction-generative.md) identifies the exact subset used by each method.
...
\qquad
s_t=\operatorname{sigmoid}\!\left(b+\sum_m w_m\widetilde a_t^{(m)}\right),
$$

where $m$ indexes modalities and $\operatorname{sigmoid}(\cdot)$ is the logistic function. A method must state whether $w_m$, temperature, and prompt were fixed, fitted without summary labels, or tuned on the benchmark; these choices change its supervision category.
