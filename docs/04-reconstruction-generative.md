...
This narrative ledger is retained from the 2026-09-06 audit. It is not a single compatible comparison group: source-specific split and decoder differences are described below. The [generated benchmark catalog](12-benchmarks.md) contains newly checked records with explicit protocol tuples. A dash in the rank columns means the paper did not report Kendall $`\tau`$ or Spearman $`\rho`$.
...
| SUM-GAN-sup | **Supervised** | 41.7 | 56.3 | — | Human-score loss; supervised variant |
...
```math
A=\mathrm{softmax}\!\left(\frac{(XW_f)(XW_g)^\top}{\sqrt{d'}}\right),
\qquad
O_i=A(XW_k),
```

```math
Y=\mathrm{Concat}(O_1,\ldots,O_h)W_h+X.
```
...
=\frac1T\sum_t|p_t-\mathrm{median}(p)|^2,
\qquad
...
SUM-FCN is primarily a supervised temporal analogue of an image-segmentation FCN, but its $`\mathrm{SUM-FCN}_{\mathrm{unsup}}`$ variant is a canonical direct-reconstruction baseline and the architectural predecessor of UnpairedVSN.
...
```math
H=\mathrm{softmax}\!\left(\frac{QK^\top}{\sqrt d}\right)V,
\qquad
S=\mathrm{sigmoid}\!\left(\mathrm{linear}(\mathrm{norm}(H))\right).
```
...
```math
h_i=\mathrm{BiLSTM}(\mathrm{Lin}(x_i),h_{i-1}),
\qquad
p_i=\mathrm{softmax}(\mathrm{Lin}(h_i)/\tau)_1.
```
...
=\left\|s_0-
\mathrm{sigmoid}\!\left(\mathrm{FC}\!\left(
f_\theta(C(u_t),t,Z)
...
| SummDiff (**supervised**) | Official author source | Python 3.10, PyTorch 2.6.0 | SumMe/TVSum split files; HDF5 inputs external | Not found |
