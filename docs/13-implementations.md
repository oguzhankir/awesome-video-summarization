...
| Inspect current three-modality fusion | [TripleSumm](https://github.com/smkim37/TripleSumm) | Distinguish raw-score correlations, fixed-five-second mAP and the unused keyshot decoder |
| Study self-supervised pretraining before supervised selection | [SSPVS](https://github.com/HopLee6/SSPVS-PyTorch) | Separate representation losses from summary-label MSE and audit held-out epoch choice |
| Study paired keyframe-caption output | [Multi-VidSum](https://github.com/cl-tohoku/Multi-VidSum), [evaluator](https://github.com/cl-tohoku/Multi-VidSum-Eval) | Trace visual alignment before caption scoring; do not substitute keyshot F1 |
| Audit a modern release for blockers | [TRINITY](https://github.com/vanilladucky/TRINITY), [SD-VSum](https://github.com/IDT-ITI/SD-VSum), [CoE](https://github.com/youxiaoxing/CoE) | Compare paper config, loader schema, dependencies and evaluator path at pinned commits |
...
### Modern and newly closed source gaps

| Implementation and attribution | Source snapshot | Artifact state | Main reproduction finding |
|---|---|---|---|
| **TripleSumm**, official | [b7a8e7e](https://github.com/smkim37/TripleSumm/tree/b7a8e7e873ac3223b0ced40b1852d30b9934ba67) | Pinned HF features and MoSu/Mr. HiSum checkpoints advertised | Main path reports correlations/fixed-shot mAP; shipped 15%-knapsack decoder is unused |
| **LLMVS**, official | [417b35bf](https://github.com/mlee47/LLMVS/tree/417b35bf4704a701d849feeef3b6277ab907a85c) | External features/embeddings/checkpoints linked; no generation source | Test-named split selects and evaluates checkpoint; visual tensors are loaded but unused by model |
| **CoE**, official | [924ed7d6](https://github.com/youxiaoxing/CoE/tree/924ed7d67935022e09a371c8114d5f25cc1759a6) | No trained artifact; external MongoDB/model services required | Pip instructions target conda export, evaluator dependencies are missing, refinement defaults off |
| **SD-VSum**, official | [d57c2be3](https://github.com/IDT-ITI/SD-VSum/tree/d57c2be3b05cb13eca3b7a3572c3df204e97c4fb) | Zenodo dataset/checkpoint with recorded hashes | Loader expects absent `gtscores`; S-News validation/test routing drops dataset/split arguments |
| **TRINITY**, official | [dc70fdec](https://github.com/vanilladucky/TRINITY/tree/dc70fdecb98de5888f71159f6ac1ea223103b9bf) | Pinned HF features/labels/splits; no checkpoints | Paper/default settings conflict; advertised baselines do not match combined solver signatures |
| **KnowVis**, official release stub | [da14209f](https://github.com/yixu-cityu/KnowVis/tree/da14209fd4a7483474fad4fefdde8f0920290976) | README and two PNGs only | No executable code, prompt/config, evaluator, dependency or license |
| **SSPVS**, official | [e4722000](https://github.com/HopLee6/SSPVS-PyTorch/tree/e472200069b1697e392f7ac278593c973026b680) | Figshare artifacts advertised, not downloaded | `test_keys` act as validation and post-hoc best-epoch source |
| **CLIP-It**, official stub | [c1818090](https://github.com/medhini/clip_it/tree/c18180905fa5a56c2297f89ae4c5ccae8d05f180) | README only | No official implementation; repository status is unavailable, not working |
| **CLIP-It**, community sketch | [afb5ef27](https://github.com/srpkdyy/CLIP-It/tree/afb5ef27d77fe4ebf95960fdb63a46bc40832a82) | Approximate forward pass | No data/loss/training/evaluator/decoder; normalized text tensor is computed then ignored |
| **Multi-VidSum**, official | [23a05397](https://github.com/cl-tohoku/Multi-VidSum/tree/23a053970824926c2ae421ab9d5956ce133348b7) | Docker/four-GPU workflow and [pinned dataset archive](https://huggingface.co/datasets/tohoku-nlp/multi-vidsum/tree/ca03b147ddb4959229b510bde8eacc55933088af) | Separate evaluator, W&B/API/data requirements and no source-repository license; not executed |
| **Multi-VidSum evaluator**, official | [d4aa2fd4](https://github.com/cl-tohoku/Multi-VidSum-Eval/tree/d4aa2fd4f1b88b65a14f2c66676fe1529572772a) | Source only | Caption scores use the reference alignment selected by AKM cosine |
| **Personalized day-long egocentric**, official | [b18da632](https://github.com/Pravin74/interact_summ_code/tree/b18da63203581c532eb4ad50ad7837fee3f300e6) | Scripts/GUI/outputs; external C3D HDF5 required | Legacy CUDA/PyTorch, manual preparation, no lockfile or license |

**Code-search gaps.** No official Cycle-SUM implementation was verified after searches for the exact method name, its authors and GitHub code, followed by the [primary paper](https://arxiv.org/abs/1904.08265). This is “not found in this audit,” not proof that no source exists. The [SUM-GAN primary paper](https://openaccess.thecvf.com/content_cvpr_2017/html/Mahasseni_Unsupervised_Video_Summarization_CVPR_2017_paper.html) was not linked to a verified author implementation; the explicitly unofficial project above should retain that label. For the original QFVS memory-network method, the [paper](https://openaccess.thecvf.com/content_cvpr_2017/papers/Sharghi_Query-Focused_Video_Summarization_CVPR_2017_paper.pdf) points to an author project for data/evaluation code; that page could not be fetched in this audit. Later UniVTG and FCSNA releases do not establish release of that original model. Exact-title/author searches also found no official code for SGWIB, Byra et al.'s zero-shot detector, Audio for Sports Highlight Detection, Semantic Action Graph, Unified Agentic Video Editing, the Wiley multimodal model, the Springer soccer system, causal online sampling or M2UR.
...
For newer systems, start from failure boundaries rather than the advertised command. LLMVS has no untouched final partition in its released path. TripleSumm's dataset loader requires every modality even for source inspection, and its reconstructed Mr. HiSum release omits unavailable videos. TRINITY's default proposed model matches the combined solver, but its advertised alternative model choices do not. SD-VSum's released schema mismatch is a blocking `KeyError`, not a small score discrepancy. CoE requires external database and serving infrastructure before its first inference, while the committed call disables the paper's optional refinement. These exact findings are stored in [resource cards](generated/resources.md).
...
VideoXum's author [feature listing](https://huggingface.co/datasets/jylins/videoxum/tree/main) exposes BLIP and VT-CLIP feature archives; its [model listing](https://huggingface.co/jylins/vtsum_blip/tree/main) exposes `vt_clip.pth`, `vtsum_tt.pth` and `vtsum_tt_ca.pth`. PGL-SUM advertises [Zenodo models](https://doi.org/10.5281/zenodo.5635735), but that record returned HTTP 429 in the **2026-09-08** browser audit. That is a rate-limit observation, not a dead-link finding; the 2026-09-19 batch audit could not resolve any external host through DNS.

TripleSumm's pinned HF releases advertise roughly 136 GB for MoSu and 63.2 GB for reconstructed Mr. HiSum plus two roughly 11 MB checkpoints; only listings were inspected. TRINITY's pinned release is public and ungated, but it distributes derived features/labels rather than raw media and its manifest counts conflict with prose. KnowVis's card reports roughly 803 MB, but its live license field is empty even though the paper prospectively states CC BY-NC-SA 4.0. Multi-VidSum's pinned card declares Apache-2.0 for its released archive and lists a 33,838,742,254-byte LFS object; only its pointer was inspected, so the declared digest is not an auditor-computed body checksum. SD-VSum's Zenodo files expose sizes and MD5 values. None of these large artifacts was downloaded in this audit.
